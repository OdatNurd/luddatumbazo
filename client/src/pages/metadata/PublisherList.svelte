<script>
  ...
  const loadGames = async () => {
    const gameListURI = `${API}/game/meta/publisher/list`;
    // await godot(0.25);
...
<LoadZone source={loadGames()} let:result>
  <DataTable data={result.data} pageSize={20} color="primary">
    ...
    <svelte:fragment slot="row" let:row>
      <td>{row.id}</td>
      <td><a href="/publishers/{row.slug}">{row.name}</a></td>
      <td>
        {#if row.bggId !== 0}
          <Link target="_blank" href="https://boardgamegeek.com/boardgamepublisher/{row.bggId}/">
